...
wordToken s = wordTokenHelper (words s)

wordTokenHelper [] = []
wordTokenHelper (x:xs) = (wordHelper "" x) ++ (wordTokenHelper xs)

wordHelper [] [] = []
wordHelper acc [] = [acc]
wordHelper acc (x:xs) = if(not (elem x punct)) then wordHelper (acc++[x]) xs
											 else [acc] ++ [[x]] ++ (wordHelper [] xs)
...
uniqueBigrams h = uniqueBigramsHelper [] h


uniqueBigramsHelper acc [] = acc
uniqueBigramsHelper acc [_] = acc

uniqueBigramsHelper acc (x:y:xs) = uniqueBigramsHelper (add (x,y) acc) (y:xs)

uniqueTrigrams h = uniqueTrigramsHelper [] h

uniqueTrigramsHelper acc [] = acc
uniqueTrigramsHelper acc [_] = acc
uniqueTrigramsHelper acc [_,_] = acc
uniqueTrigramsHelper acc (x:y:z:xs) = uniqueTrigramsHelper(add (x,y,z) acc) (y:z:xs)
...
add x [] = [x]
add x (y:ys) = if x==y then ys ++ [x]
						else [y] ++ (add x ys)
						
	


bigramsFreq x = bigramsFreqHelper [] x

bigramsFreqHelper acc [] = acc
bigramsFreqHelper acc [x] = acc

bigramsFreqHelper acc (x:y:xs) = bigramsFreqHelper (addFreq (x,y) acc) (y:xs)


trigramsFreq x = trigramsFreqHelper [] x

trigramsFreqHelper acc [] = acc
trigramsFreqHelper acc [x] = acc
trigramsFreqHelper acc [x,y] = acc

trigramsFreqHelper acc (x:y:z:xs) = trigramsFreqHelper (addFreq (x,y,z) acc) (y:z:xs)
...
addFreq x ((y,f):ys) = if(x==y) then ys ++ [(x,f+1)] 	
			else (y,f) : (addFreq x ys)


getFreq a [] =0
getFreq a ((x,f):xs)= if a==x then f else getFreq a xs

generateOneProb ((a,b,c),f1) l = if(f2==0) then 0
				 else (f1/f2) 
				 where f2 = getFreq (a,b) l


genProbPairs [] l = []
genProbPairs ((x,f):xs) l = (x,(generateOneProb (x,f) l)):genProbPairs xs l 

generateNextWord x y  = 
			if (length m == 0) then error("Sorry, it is not possible to infer from current database") 
			else m!!a
		        where (m,a) = (generateNextWordHelper [] x y,randomZeroToX ((length m)-1) )
		  

generateNextWordHelper acc x [] = acc
generateNextWordHelper acc [x,y] (((a,b,c),f):ys) = if (x==a&&y==b&&f>0.03) then generateNextWordHelper (acc++[c]) [x,y] ys 
								            else generateNextWordHelper acc [x,y] ys
	
generateText x n = generateTextHelper out props n
		   where (out,theWords,biFreq,triFreq,props) = (wordToken x,wordTokenList docs,bigramsFreq theWords,trigramsFreq theWords,genProbPairs triFreq biFreq)

generateTextHelper out _ 0 = displayText out True
generateTextHelper out props n = generateTextHelper (out++[generateNextWord [x,y] props]) props (n-1)  
				 where (len ,y,x)=(length out , out!!(len-1),out!!(len-2))
displayText [] firstIt = []
displayText (x:xs) firstIt = if(length x == 1&& elem (x!!0) punct) 
			then x ++ (displayText xs False)
			else if (not firstIt )
				then (" "++x) ++ (displayText xs False)
				else x ++ (displayText xs False)
...
sentTokenHelper [] []  =[]
sentTokenHelper [] acc =[acc]
sentTokenHelper (x:xs) acc = if (elem x p ) then [acc++[x]]++(sentTokenHelper  xs [])
                             else if((length acc)==0&&x==' ') then sentTokenHelper  xs (acc) 
                             else sentTokenHelper  xs (acc++[x]) 
